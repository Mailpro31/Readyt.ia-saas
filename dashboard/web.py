...
class WebDashboard:
    ...
    # ── Detailed health (Phase 1.4) ──────────────────────────

    def _detailed_health(self) -> dict:
        """Aggregate job freshness, recent errors, and account health."""
        jobs = []
        try:
            ...
            logger.debug("health: could not list jobs: %s", e)
        ...
        try:
            ...
            errors_24h = row["c"] if row else 0
        except Exception as e:
            logger.debug("health: error count failed: %s", e)

        accounts = []
        try:
            for h in self.orch.account_mgr.get_all_health():
                username = h.get("username", "")
                karma = self.orch.account_mgr.get_cached_karma(username)
                warm = None
                try:
                    warm = self.orch.db.get_warmup(username, h.get("platform", "reddit"))
                except Exception:
                    pass
                accounts.append({
                    **h,
                    "karma": karma,
                    "warmup_status": warm.get("status") if warm else None,
                    "warmup_day": warm.get("day_number") if warm else None,
                })
        except Exception as e:
            logger.debug("health: account health failed: %s", e)
        ...
        return {
            ...
            "accounts": accounts,
            "geo_enabled": getattr(self.orch, "geo_tracker", None) is not None
            and self.orch.geo_tracker.enabled,
        }
    ...
    def _setup_routes(self):
        ...
        # ── GET /health/detailed (Phase 1.4) ───────────────
        @app.get("/health/detailed")
        async def health_detailed():
            """Job freshness, 24h error count, and per-account health."""
            return self._detailed_health()

        # ── GET /r/{slug} — tracked link redirect (Phase 1.1) ─
        @app.get("/r/{slug}")
        async def tracked_redirect(slug: str, request: Request):
            """Record a click and 302-redirect to the real product URL."""
            link = self.orch.db.get_tracked_link(slug)
            if not link:
            ...
            try:
                ...
                ip_hash = (
                    hashlib.sha256(ip.encode()).hexdigest()[:16] if ip else ""
                )
                self.orch.db.record_link_click(
                    slug, referer=ref, user_agent=ua, ip_hash=ip_hash,
                )
                logger.info("Tracked click: slug=%s -> %s", slug, link["target_url"])
            except Exception as e:
                logger.error("Failed to record click for %s: %s", slug, e)
            return RedirectResponse(url=link["target_url"], status_code=302)

        # ── GET /api/links — tracked links + click counts ──
        @app.get("/api/links")
        async def get_links(
            project: Optional[str] = None, _=Depends(self._verify_token),
        ):
            try:
                return self.orch.db.get_tracked_links_with_clicks(
                    project=project, limit=200,
                )
            except Exception as e:
                return {"error": str(e)}

        # ── GET /api/geo — AI citation results (Phase 1.2) ─
        @app.get("/api/geo")
        async def get_geo(
            project: Optional[str] = None, _=Depends(self._verify_token),
        ):
            try:
                return {
                    "citations": self.orch.db.get_geo_citations(
                        project=project, days=90,
                    ),
                    "rates": (
                        self.orch.db.get_geo_citation_rate(project, days=90)
                        if project else []
                    ),
                }
            except Exception as e:
        ...
        # ── GET /api/warmup — warm-up progress (Phase 1.0) ─
        @app.get("/api/warmup")
        async def get_warmup(_=Depends(self._verify_token)):
            try:
                return self.orch.warmup.progress_view()
            except Exception as e:
                return {"error": str(e)}
