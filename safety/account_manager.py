...
class AccountManager:
    ...
    def get_next_account(self, platform: str, project: Optional[str] = None) -> Optional[Dict]:
        ...
        with self._lock:
            ...
            if project:
                ...
                # Warm-up gate (Phase 1.0): a project-targeted (product) action
    ...
    def is_warmup_ready(self, username: str, platform: str = "reddit") -> bool:
        """Return True if the account is cleared to run product actions.

        Ready when there is no warm-up row (established account, backward compat)
        OR the warm-up status is 'ready'. A 'warming'/'paused' account returns
        False so it is never used for project-related content.
        """
        try:
            row = self.db.get_warmup(username, platform)
        except Exception:
            return True  # never block over a bookkeeping error
        if row is None:
            return True
