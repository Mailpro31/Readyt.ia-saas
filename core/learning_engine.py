...
class LearningEngine:
    ...
    def learn(self, project: str = ""):
        ...
        for proj in projects:
            ...
            self._learn_from_real_conversions(proj)
    ...
    def _learn_from_real_conversions(self, project: str):
        """Phase 1.5: fold REAL attribution data into the learned weights.

        Real clicks (from link_tracker) and AI citations (from geo_tracker) are
        a stronger performance signal than the LLM engagement heuristic, so a
        subreddit that produces actual clicks gets its weight boosted — which
        strategy.py already consumes via the 'subreddit' weight category.

        The boost is multiplicative on top of the existing engagement-derived
        weight, and only applies once enough real clicks exist, so it augments
        rather than replaces the heuristic where data is still sparse.
        """
        try:
            click_rows = self.db.get_clicks_by_subreddit(project, days=60)
        except Exception as e:
            logger.debug("conversion learning: no click data (%s)", e)
            click_rows = []
        ...
        for row in click_rows:
            ...
            if not sub or clicks <= 0:
                continue
            # Conversion rate per tracked link in this subreddit.
            links = max(row.get("links", 1), 1)
            conv = clicks / links
            # Boost factor: 1.0 baseline, up to ~+150% for strong converters.
            boost = 1.0 + min(conv, 3.0) * 0.5

            existing = self.db.get_learned_weight("subreddit", sub, project)
            base = existing.get("weight", 1.0) if existing else 1.0
            new_weight = round(max(0.1, base * boost), 3)
            self.db.update_learned_weight(
                category="subreddit",
                key=sub,
                project=project,
                weight=new_weight,
                sample_count=(existing.get("sample_count", 0) if existing else 0)
                + clicks,
                avg_engagement=(
                    existing.get("avg_engagement", 0.0) if existing else 0.0
                ),
            )
            logger.info(
                "Conversion signal: r/%s %d clicks → weight %.2f×%.2f=%.2f "
                "for %s", sub, clicks, base, boost, new_weight, project,
            )

        # Global GEO signal: rising citation rate reinforces promotional posture.
        try:
            geo_rates = self.db.get_geo_citation_rate(project, days=60)
            if geo_rates:
                total = sum(g.get("total", 0) for g in geo_rates)
                cited = sum((g.get("cited_count") or 0) for g in geo_rates)
                if total:
                    rate = cited / total
                    logger.info(
                        "GEO citation rate for %s: %.0f%% (%d/%d) — "
                        "feeding learning loop", project, rate * 100, cited, total,
                    )
                    self.db.update_learned_weight(
                        category="geo_citation_rate",
                        key="ai_visibility",
                        project=project,
                        weight=round(rate, 3),
                        sample_count=total,
                        avg_engagement=round(rate, 3),
                    )
        except Exception as e:
            logger.debug("GEO learning skipped: %s", e)
