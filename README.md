...
| **Warm-up** | 30 min | Active account warm-up cycle (Nova 1.0) |
| **GEO Track** | 48h | AI citation tracking (Nova 1.2, paid APIs) |
| **First-click Alert** | 10 min | Telegram alert on first tracked-link click |

---

## Nova Extension (Phase 1)

This fork extends MiloAgent to drive Nova's Reddit promotion
([novaspeak.app](https://novaspeak.app)) with SaaS-grade attribution and
observability, keeping the `cost_mode: zero` spirit wherever possible.

### 1.0 — Active account warm-up

New Reddit accounts cannot safely post product content. `add-account` enrolls
an account in a progressive multi-day warm-up automatically:

```bash
python3 miloagent.py add-account -u my_reddit_user -p 'password' --project Nova
```

The account is inserted with `status=warming` and the orchestrator drives
neutral actions (generic upvotes / bland comments on product-unrelated
subreddits, configured in `config/warmup.yaml`). Once **both** the minimum
karma and minimum warm-up days are reached, it is **auto-graduated to `ready`**
— no manual step — and only then does `account_manager` include it in the Nova
account pool. Config: `config/warmup.yaml`. Table: `warmup_progress`.
Progress API: `GET /api/warmup` ("Jour 6/10" style progress + ETA).

### 1.1 — Lead attribution (link tracking)

When generated content contains a Nova URL, `core/link_tracker.py` mints a short
unique slug, stores which thread/account/subreddit produced it, and rewrites the
raw URL into a tracked redirect. Clicks are logged by `GET /r/{slug}` (HTTP 302
to the real URL with `?utm_source=reddit&utm_content={slug}`).

- Enable with `features.link_tracking: true` (default) in `settings.yaml`.
- Set the product base URL per project via `project.base_url`.
- Tables: `tracked_links`, `link_clicks`. Dashboard API: `GET /api/links`.

> **Infra dependency (not assumed to exist):** the redirect service must run on
> a **publicly reachable** host. Point a subdomain such as
> `https://track.novaspeak.app` (or reverse-proxy `/r/` on `novaspeak.app`) at
> this dashboard and set `link_tracking.track_base_url` in `settings.yaml`. If
> left empty, slugs are stored with a relative `/r/{slug}` path (dev/test only).

### 1.2 — AI citation tracking (GEO / AEO)

`core/geo_tracker.py` periodically (48h) asks each configured AI provider a set
of probe queries (e.g. *"best speech to text app for windows"*) and records
whether Nova / novaspeak.app is mentioned. Table: `geo_citations`. API:
`GET /api/geo`.

- Copy `config/geo.yaml.example` → `config/geo.yaml` and add provider API keys
  (or set the `*_API_KEY` env vars). Probe queries are keyed by project filename.
- Brand terms counted as a citation come from each project's
  `project.geo.brand_terms` (`geo_tracker` also recognises `Nova`/`novaspeak`,
  with guards against false positives like "Nova Launcher"/"Nova Scotia").

> **💰 GEO tracking cost (this feature is NOT zero-cost):** OpenAI, Anthropic and
> Perplexity have no sufficient free tier. Estimate with the default 6 probe
> queries × 3 enabled providers, every 48h (~15 runs/month) ≈ **270 calls/month**.
> Each call is one short prompt + a ~1k-token answer. At current small-model
> pricing (~gpt-4o-mini / claude-haiku / sonar, roughly $0.15–$1 per 1M input
> and $0.60–$5 per 1M output tokens) that lands around **$1–$4 / month total**.
> Add/remove providers or queries in `config/geo.yaml` to scale the cost.

### 1.3 — Tests & smoke-test

```bash
python3 -m pytest tests/ -q                # unit tests (link_tracker, geo_tracker)
python3 scripts/smoke_test_phase1.py       # end-to-end PASS/FAIL, exit code
```

The smoke-test inserts a tracked link, exercises the real `/r/{slug}` 302
redirect, runs a stubbed GEO dry-run, and verifies the full warm-up flow
(add → warming → auto-graduate → actionable for Nova) with no manual step.

### 1.4 — Health check & alerts

- `GET /health/detailed`: per-job next-run, 24h error count, per-account health
  (karma, cooldown, warm-up status/day).
- Telegram alerts on: first click of any tracked link, first AI citation for a
  probe query, and account warm-up graduation.

### 1.5 — Real-conversion feedback loop

`learning_engine._learn_from_real_conversions` folds real click data
(per-subreddit) and GEO citation rate into the learned `subreddit` weights that
`strategy.py` already consumes — real conversion progressively augments the LLM
engagement heuristic.

### 1.6 — Weekly report

The existing Sunday Telegram report now includes tracked clicks (total + best
converting subreddits), AI-citation rates per provider, and 2–3 actionable
recommendations.
...
| `add-account -u <user> -p <pass> [--project Nova]` | Add account + auto-enroll in warm-up |
